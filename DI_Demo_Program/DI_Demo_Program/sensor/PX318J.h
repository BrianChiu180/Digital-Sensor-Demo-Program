...
class CPX318J : public CSensorInfo {
public:
	CPX318J()
	{
		ModelName = _T("PX318J");
		pDieInfo = new CDA3380;
#if defined(_PX318J_10000) || defined(_DEBUG) || defined(_PX318J)
		ModelVersion.insert(10000);//1.0 DA3380
#endif
#if defined(_PX318J_10100) || defined(_DEBUG) || defined(_PX318J)
		ModelVersion.insert(10100);//1.01 DA3380
#endif


#if defined(_DEBUG) || defined(_PX318J)
	...
	void Init(uint16_t version)
	{
		...
		case 10000: {
			...
			pDieDetail->DlgName[TAB_2] = _T("PS + IR");
			...
			pRegTable = &pDieDetail->RegisterTable;
			{
				...
				pRegTable->RegInfo.at(0xFF).bEngMode = FALSE;
				{
					...
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
						pCtrlDetail->CtrlName = make_pair(_T("IrRstErr"), _T("IrRstErr"));
						pCtrlDetail->CtrlType = LED;
						pCtrlDetail->BitMapping[0] = _T("IrRstErr");
					}
...
#else
						for (uint8_t i = 1; i < 51; i++)//最大值暫定在50
#endif
						{
			...
			pRegTable->BitDetail.at(_T("IrEn")).DefaultVal.at(0) = 0x00;
			pRegTable->BitDetail.at(_T("PsDrv")).DefaultVal.at(0) = 11;	// 12 mA
			pRegTable->BitDetail.at(_T("PsBits")).DefaultVal.at(0) = 1;	// 10 bit
			pRegTable->BitDetail.at(_T("PsPuw")).DefaultVal.at(0) = 32;	// 1W
			pRegTable->BitDetail.at(_T("PsPuc")).DefaultVal.at(0) = 2;	// 2C
			...
			//RD better setting
			pRegTable->BitDetail.at(_T("PsCap")).DefaultVal.at(0) = 5;
			pRegTable->BitDetail.at(_T("IrCap")).DefaultVal.at(0) = 5;
			pRegTable->BitDetail.at(_T("PsGain")).DefaultVal.at(0) = 5;
			pRegTable->BitDetail.at(_T("PsPrePuc")).DefaultVal.at(0) = 0;
			pRegTable->BitDetail.at(_T("IrGain")).DefaultVal.at(0) = 0;
			pRegTable->BitDetail.at(_T("IrPuc")).DefaultVal.at(0) = 2;	// 2C
		...
		case 10100: {
			...
			pRegTable = &pDieDetail->RegisterTable;
			{
				...
				pRegTable->RegInfo.at(0xFE).bEngMode = FALSE;
				{
					...
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
						pCtrlDetail->CtrlName = make_pair(_T("PsInt"), _T("PsInt"));
						pCtrlDetail->CtrlType = LED;
						pCtrlDetail->BitMapping[0] = _T("PsInt");						
					}
			...
			pReadTable = &pDieDetail->RegisterTable.ReadTable;
#ifdef _RD
			pReadTable->insert(CReadTable(0xFB, 1));
#endif
			pReadTable->insert(CReadTable(0xFE, 2));
