...
class CCP507C : public CSensorInfo {
public:
	CCP507C()
	{
		ModelName = _T("CP507C");
		pDieInfo = new CDA3107;
#if defined(_CP507C_2000) || defined(_DEBUG)
		ModelVersion.insert(2000);
#endif
#if defined(_CP507C_6100) || defined(_DEBUG) || defined(_CP507C)
		ModelVersion.insert(6100);
#endif
	}
		...
		{	
			...
			pRegTable = &pDieDetail->RegisterTable;
			{
				...
				pRegTable->RegInfo.at(0x02).bEngMode = FALSE;
				{
					...
					DlgCheck.Init(TAB_4, make_pair(6, 1));
					ASSERT(DlgCheckSet.insert(DlgCheck).second);
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
						pCtrlDetail->CtrlType = CHECK_GROUP;
				...
				pRegTable->RegInfo.at(0x3E).bEngMode = FALSE;
				{
					...
					DlgCheck.Init(TAB_3, make_pair(6, 6));
					ASSERT(DlgCheckSet.insert(DlgCheck).second);
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
						pCtrlDetail->CtrlType = BUTTON_GROUP;
			...
			pRegTable->BitDetail.at(_T("PLASER")).DefaultVal.at(0) = 0x00;
#if defined(_DEBUG)
			pRegTable->BitDetail.at(_T("IREN")).DefaultVal.at(0) = 0x01;
			pRegTable->BitDetail.at(_T("IRTIME")).DefaultVal.at(0) = 0x0F;
			pRegTable->BitDetail.at(_T("IRPUW")).DefaultVal.at(0) = 0x3f;
			pRegTable->BitDetail.at(_T("IRMEAN")).DefaultVal.at(0) = 0x00;
#endif
			pReadTable = &pDieDetail->RegisterTable.ReadTable;
			pReadTable->clear();
			pReadTable->insert(CReadTable(0x01, 1));
			pReadTable->insert(CReadTable(0x1E, 1));
#if defined(_DEBUG)
			pReadTable->insert(CReadTable(0x28, 12, LogDec));
#else
			pReadTable->insert(CReadTable(0x28, 2, LogDec));
			pReadTable->insert(CReadTable(0x2C, 8, LogDec));
#endif
