...
class CPX328J : public CSensorInfo {
public:
	CPX328J()
	{
		ModelName = _T("PX328J");
		pDieInfo = new CDA3380;
#if defined(_PX328J_10000) || defined(_DEBUG) || defined(_PX328J)
		ModelVersion.insert(10000);//1.0 DA3380
#endif


#if defined(_DEBUG) || defined(_PX328J)
	...
	void Init(uint16_t version)
	{
		...
		case 10000: {
			...
			pRegTable = &pDieDetail->RegisterTable;
			{
				...
				pRegTable->RegInfo.at(0x64).bEngMode = FALSE;
				{
					...
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
						...
						for (uint8_t i = 1; i < 0x06; i++)
						{
							csTemp.Format(_T("%.1f mA"), (float)(i) * 8.0f);
							pCtrlDetail->NameMapping[i] = csTemp;
						}
					}
#endif // _RD
			...
			pRegTable->BitDetail.at(_T("PsDrvMax")).DefaultVal.at(0) = 1;	// 120 mA
			pRegTable->BitDetail.at(_T("PsDrv")).DefaultVal.at(0) = 4;	// 32 mA
			pRegTable->BitDetail.at(_T("PsBits")).DefaultVal.at(0) = 1;	// 10 bit
			pRegTable->BitDetail.at(_T("PsPuw")).DefaultVal.at(0) = 64;	// 1W
			pRegTable->BitDetail.at(_T("PsPuc")).DefaultVal.at(0) = 31;	// 2C
