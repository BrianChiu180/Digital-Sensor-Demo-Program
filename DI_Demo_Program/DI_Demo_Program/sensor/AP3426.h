...
class CAP3426 : public CSensorInfo {
public:
	CAP3426()
	{
		ModelName = _T("AP3426");
		pDieInfo = new CDA3221;
#if defined(_AP3426_28000) || defined(_DEBUG) || defined(_AP3426)
		ModelVersion.insert(28000);
#endif
	}
	...
	void Init(uint16_t version)
	{
		...
		case 28000: {
			...
			pRegTable = &pDieDetail->RegisterTable;
			{
				...
				pRegTable->RegInfo.at(0x10).bEngMode = FALSE;
				{
					...
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
						...
						pCtrlDetail->NameMapping[0] = _T("32768 Lux / Resolution bit");
						pCtrlDetail->NameMapping[1] = _T("8192 Lux / Resolution bit");
						pCtrlDetail->NameMapping[2] = _T("2048 Lux / Resolution bit");
						pCtrlDetail->NameMapping[3] = _T("512 Lux / Resolution bit");
					}
				}
				...
				pRegTable->RegInfo.at(0x21).bEngMode = FALSE;
				{
					...
					{
						pCtrlDetail->CtrlType = COMBO_GROUP;
						pCtrlDetail->BitMapping[0] = _T("PS LED Driver");
			...
			pLuxCalculate = &pDieDetail->LuxCalculate;
			{
				...
				pLuxCalculate->Count2LuxMapping[0] = make_pair(0.5, 0.0);
			}
