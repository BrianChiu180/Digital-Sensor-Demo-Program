...
class CAP35AM : public CSensorInfo {
public:
	CAP35AM() {
		ModelName = _T("AP35AM");
#if defined(_AP35AM_10100) || defined(_DEBUG) || defined(_AP35AM)
	...
	void Init(uint16_t version) {
		...
		case 10100: {
			...
			pDieDetail->DlgName[TAB_4] = _T("IR");
			...
			pRegTable = &pDieDetail->RegisterTable;
			{
				...
				pRegTable->RegInfo.at(0x08).bEngMode = FALSE;
				{
					...
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
						pCtrlDetail->CtrlName = make_pair(_T("Als2"), _T("AlsData2"));
						pCtrlDetail->CtrlType = PROGRESSBAR;
						pCtrlDetail->CtrlColor.first = IR_BAR;
				...
				{
					DlgCheck.Init(MODEL_1, make_pair(6, 0));
					ASSERT(DlgCheckSet.insert(DlgCheck).second);
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
				...
				pRegTable->RegInfo.at(0x50).bEngMode = FALSE;
				{
...
#ifdef _DEBUG
						pCtrlDetail->NameMapping[7] = _T("x128");
#endif
					}
					...
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
						pCtrlDetail->CtrlName = make_pair(_T("AlsGain2"), _T(""));
						pCtrlDetail->CtrlType = COMBO_GROUP;
						pCtrlDetail->BitMapping[0] = _T("AlsGain2");
						pCtrlDetail->NameMapping[0] = _T("x1");
				...
				pRegTable->RegInfo.at(0x55).bEngMode = FALSE;
				{
					...
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
						...
						pCtrlDetail->CtrlFunc = CTRLFUNC_CHANGE_CONVERSION_TIME;
						pCtrlDetail->CtrlFunc |= CTRLFUNC_CALCULATE_POWER;
						pCtrlDetail->CtrlName = make_pair(_T("ALS Puw0"), _T(""));
				...
				pRegTable->RegInfo.at(0x56).bEngMode = FALSE;
				{
					...
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
						...
						pCtrlDetail->CtrlFunc = CTRLFUNC_CHANGE_CONVERSION_TIME;
						pCtrlDetail->CtrlFunc |= CTRLFUNC_CALCULATE_POWER;
						pCtrlDetail->CtrlName = make_pair(_T("ALS Puw1"), _T(""));
						pCtrlDetail->BitMapping[0] = _T("AlsPuw1");
						pCtrlDetail->NameMapping[0] = _T("Disable AlsData1, Default");
						for (uint8_t i = 1; i < 128; i++)
						{
							csTemp.Format(_T("%d ms"), i);
							pCtrlDetail->NameMapping[i] = csTemp;
				...
				pRegTable->RegInfo.at(0x57).bEngMode = FALSE;
				{
					...
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
						...
						pCtrlDetail->CtrlFunc = CTRLFUNC_CHANGE_CONVERSION_TIME;
						pCtrlDetail->CtrlFunc |= CTRLFUNC_CALCULATE_POWER;
						pCtrlDetail->CtrlName = make_pair(_T("ALS Puw2"), _T(""));
				...
				pRegTable->RegInfo.at(0x5B).bEngMode = FALSE;
				{
					...
					pCtrlDetail = &pCtrlInfo[DlgCheck.DlgSel]->CtrlDetail[DlgCheck.CtrlXY];
					{
						pCtrlDetail->CtrlType = COMBO_GROUP;
						pCtrlDetail->CtrlFunc = CTRLFUNC_SELDATA;
						pCtrlDetail->CtrlName = make_pair(_T("ALS INT Source"), _T(""));
						...
						pCtrlDetail->RegMapping[1] = 0x06;
#ifdef _DEBUG
						pCtrlDetail->NameMapping[2] = _T("Als Data2");
						pCtrlDetail->RegMapping[2] = 0x08;
#endif
					}
#ifdef _DEBUG
					...
					{
						pCtrlDetail->CtrlType = COMBO_GROUP;
						pCtrlDetail->CtrlName = make_pair(_T("ALS Turbo"), _T(""));
						pCtrlDetail->BitMapping[0] = _T("AlsTurbo");
						pCtrlDetail->NameMapping[0] = _T("10/10 Time Unit");
						pCtrlDetail->NameMapping[1] = _T("1/10 Time Unit");
					}
#endif
			...
			pRegTable->BitDetail.at(_T("AlsPuw1")).DefaultVal.at(0) = 10;
			pRegTable->BitDetail.at(_T("AlsPuw2")).DefaultVal.at(0) = 0;
			...
			pReadTable->insert(CReadTable(0x00, 8, LogDec));
#ifdef _DEBUG
